...
object Other:
  trait StorageContainerInterface:
    def put[A : JsonValueCodec](id: String)(value: A): Future[Unit]
        ...
        container.items.upsert(JsAggregateContainer(
          ...
          .andThen {
            case scala.util.Success(response) =>
              services.logger.trace(s"AggregateRepository.put, Put aggregate: $id - ${response.statusCode}")
            case scala.util.Failure(exception) =>
              services.logger.error(s"AggregateRepository.put, Put aggregate: $id - ${exception.getMessage()}")
          }
      )
...
class StorageService(
  ...
  private val cosmosClient = CosmosClient(services.config.cosmosDBConnectionString)
