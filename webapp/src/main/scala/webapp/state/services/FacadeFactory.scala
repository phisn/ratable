...
class FacadeFactory(services: {
  ...
  def registerAggregate[A : JsonValueCodec : Bottom : Lattice](aggregateTypeId: String): Facade[A] =
    val actions = Evt[A => A]()
    
    val aggregateSignalInFuture = services.statePersistence
      // For singleton aggregates we use the aggregate type id as the id
      .loadAggregate(aggregateTypeId, aggregateTypeId)
      ...
      .map(services.aggregateFactory.createAggregateSignal(actions))
